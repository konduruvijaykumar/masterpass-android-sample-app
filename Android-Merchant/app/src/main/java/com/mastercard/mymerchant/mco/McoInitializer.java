...
public class McoInitializer {
    ...
    /**
     * Method that creates config object and initialize MCO-SDK library mock version
     *
     * @param applicationContext app context
     * @throws MasterPassException
     */
    public void initMockMCO(final Context applicationContext) throws MasterPassException {
        Log.d(TAG, "initMockMCO");
        ...
        // For mock use mock session signing key
        MasterPass.getInstance().init(
                applicationContext,
                masterPassMerchantConfig,
                requestMockSessionKeySigning(applicationContext),
    ...
    private final InitializationListener mInitializationListener = new InitializationListener() {
        ...
        public void initializationComplete() {
            Log.i(TAG, "Init MCO: initializationComplete");
            DataManager.getInstance().setMcoInitialized(true);
        }

        @Override
        public void initializationStarted() {
            Log.i(TAG, "Init MCO: initializationStarted");
        }
    };
