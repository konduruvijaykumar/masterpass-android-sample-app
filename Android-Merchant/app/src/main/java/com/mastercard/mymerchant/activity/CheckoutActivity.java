...
import com.mastercard.mymerchant.MyMerchantMasterPassButtonListener;
    ...
    private void initMCOCheckout() {
        ...
        if (mDataManager.isMcoInitialized()) {
            ...
            try {
                ...
                MasterPassButtonListener transactionDetailsListener = new MyMerchantMasterPassButtonListener() {
                    @Override
                    public TransactionDetails getTransactionDetails() {
                        Currency currencyObj = Currency.getInstance(mDataManager.mBasket.getCurrencyCode());
                        try {
                            Log.v(TAG, "Getting transaction details...");
                            ...
                            transactionDetails.setAmountData(
                                    new AmountData(
                                            CurrencyUtils.convertToText(mDataManager.mBasket.getSubTotal(), currencyObj, false),
                                            CurrencyUtils.convertToText(0, currencyObj, false), // Tax
                                            CurrencyUtils.convertToText(mDataManager.mBasket.getSubTotal(), currencyObj, false),
                                            currencyObj));
                            return transactionDetails;
                        } catch (MasterPassException e) {
                            Log.e(TAG, "Setting AmountData exception: " + e.getMessage() + " " +
